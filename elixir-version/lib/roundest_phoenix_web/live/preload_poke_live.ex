defmodule RoundestPhoenixWeb.PRELOADPokeLive do
  ...
  def render(assigns) do
    ...
    <div class="w-full grow flex flex-col items-center justify-center gap-8">
      ...
      <div class="hidden">
        <.async_result :let={[nextFirstEntry, nextSecondEntry]} assign={@nextEntries}>
  ...
  def handle_event("vote", %{"winner_id" => winner_id, "loser_id" => loser_id}, socket) do
    ...
    {:noreply,
     socket
     |> assign(:entries, socket.assigns.nextEntries)
     |> assign_random_pair(:nextEntries)}
  end

  def mount(_params, _session, socket) do
    {:ok,
     socket
     |> assign_random_pair(:entries)
     |> assign_random_pair(:nextEntries)}
  end
  ...
  defp assign_random_pair(socket, name) do
    assign_async(socket, [name], fn ->
      {:ok, %{name => get_random_pair()}}
    end)
  end
